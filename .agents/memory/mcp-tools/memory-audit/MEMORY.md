...
scopePath: null
---
# mcp-tools.memory-audit — Module Memory

## Current Truth

- This card is the schema v2 memory owner for mcp-tools.memory-audit.
...
- source:src/memory-audit.ts
- source:src/memory-compatibility.ts
- source:src/tests/memory-audit.test.ts

## Read Contract

- Read this card before editing mcp-tools.memory-audit tracked files or changing their ownership boundaries.
...
- mcp-tools.memory-audit 已升級為 schema v2 主卡。
- 舊版決策與課題已完整保存到 archive-001.md。
- 主卡只保留目前有效真相、限制、週期事件與追蹤檔案。
- 目前沒有硬性拆分阻擋。
- memory_audit 不會把記憶歸檔卷計入未歸屬產品檔案。
- memory_audit 的未歸屬統計與 VS Code/MCP 其他入口共用同一套排除規則。
- 後續修改此卡時應先讀最新原始碼。
