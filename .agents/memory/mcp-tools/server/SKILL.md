...
> [!CAUTION]
> 🟠 **系統強制攔截**：此記憶已過期失真！
> 追蹤檔案異動：`src/mcp-server.ts`、`src/tests/mcp-server.test.ts`（2026-06-14T23:07:24+08:00）
...
- The MCP server now reports release version 5.4.3 through both server metadata and the CLI version flag.
...
- 02: Synced the MCP server release version and CLI coverage after the 5.4.3 publish.
...
- 目前沒有硬性拆分阻擋。
- MCP server 目前對外版本為 5.4.3，CLI 版本查詢與測試已同步。
- 後續修改此卡時應先讀最新原始碼。
