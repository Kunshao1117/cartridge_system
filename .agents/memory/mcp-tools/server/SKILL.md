...
- D05: v5.4 MCP server metadata 版本同步為 5.4.0；版本升級時仍需同時檢查 package.json 與此硬編碼 SDK metadata。
...
- D10: v5.4.0 發布目標包含 npm runtime，因此 `MCP_SERVER_VERSION` 與 package version 同步為 5.4.0，避免 `cartridge-system --version` 顯示舊版。
