...
- 桌面監控台未歸屬判定 — 記憶歸檔卷與記憶系統內部檔案不再被列為產品未歸屬檔案，桌面快照數字與 VS Code 外掛、MCP 工具保持一致。

## [5.4.3] — 2026-06-04

### fix

- 未歸屬檔案判定 — 記憶歸檔卷與記憶系統內部檔案不再被列為產品未歸屬檔案；狀態列、健康報告、治理側欄、機櫃工作台、MCP 摘要與提交預檢共用同一套乾淨索引視圖。
- 舊索引殘留清理 — 掃描、讀取、持久化與監聽事件會排除記憶內部產物；Git dirty/untracked 仍照常保留，不會被記憶健康邏輯隱藏。

### chore

- 記憶與專案 DNA — 同步 schema v2 記憶卡、歸檔卷與專案 DNA。
- 測試覆蓋 — 補上記憶歸檔卷殘留回歸案例，總測試更新為 37 個測試檔案、285 個案例。

## [desktop-v5.4.2] — 2026-06-04

### fix

- 桌面操作回饋 — 所有桌面按鈕、列表選取、導引開檔、設定切換與複製提示都會回寫底部狀態列；移除監控專案前會先要求確認，取消也會顯示結果。
- 桌面滾輪維修 — 左側專案清單、中間待處理區、右側詳情、卡匣列表、未歸屬列表、問題導引與設定面板統一回到原生垂直捲動；未歸屬檔案不再截斷前 12 筆。

### chore

- 桌面建構驗證 — 補強桌面 renderer 靜態回歸測試，確認狀態回饋、移除確認、原生滾動容器與未歸屬清單完整可達。

## [5.4.2] — 2026-06-04

### feat

- 記憶壓縮治理 — 新增 schema v2 記憶卡度量，索引與 MCP 摘要會揭露卡片大小、行數、中文比例、週期事件數、舊格式狀態、平面歸檔卷與建議動作；週期事件滿 30 筆、主卡超限、根層索引超 8KB 或歸檔卷超限時，開工摘要與提交預檢列為阻擋。
- 懶升級與拆分提示 — 舊格式卡維持可讀，`memory_audit` 與 `memory_commit` 只回報待升級或先彙整警告，不自動替 AI 改寫主卡內容；tracked files 超過 8 只列為 split advisory，不阻擋提交。

### fix

- 桌面操作與滾輪 — 補齊桌面監控台操作狀態列、選取回饋、移除確認、開檔錯誤回報與七個主要滾動區的原生捲動行為。

### chore

- 版本升級 — 套件版本、鎖定檔、README 安裝與發布範例、MCP runtime 對外版本與版本測試同步至 5.4.2。
- 測試覆蓋 — 新增記憶壓縮度量、29/30/31 筆週期事件、清單欄位、提交同步阻擋、平面歸檔卷、舊式 archive 遷移、開工摘要、提交預檢拆卡 advisory、桌面操作回饋與滾輪案例，總測試案例提升至 279 個。
