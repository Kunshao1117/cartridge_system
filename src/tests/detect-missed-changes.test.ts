...
describe("detectMissedChanges — 離線變動偵測", () => {
  ...
  it("記憶卡無更新時間時應跳過該卡匣", () => {
    ...
    expect(
      manager.getIndex().cartridges["mem-no-date"].pendingChanges,
    ).toHaveLength(0);
  });
  ...
  it("多檔案混合場景：部分新、部分舊", () => {
    ...
    vi.mocked(fs.statSync)
      .mockReturnValueOnce({
        mtimeMs: toMs("2026-03-29T12:00:00+08:00"),
      } as unknown as ReturnType<typeof fs.statSync>) // new.ts — 較新
      .mockReturnValueOnce({
        mtimeMs: toMs("2026-03-27T08:00:00+08:00"),
      } as unknown as ReturnType<typeof fs.statSync>) // old.ts — 較舊
      .mockReturnValueOnce({
        mtimeMs: toMs("2026-03-29T15:00:00+08:00"),
      } as unknown as ReturnType<typeof fs.statSync>); // also-new.ts — 較新

    manager.detectMissedChanges(DEFAULT_SCORING);

    const entry = manager.getIndex().cartridges["mem-mixed"];
    expect(entry.pendingChanges).toHaveLength(2);
    expect(entry.pendingChanges.map((c) => c.filePath)).toEqual([
      "src/new.ts",
      "src/also-new.ts",
    ]);
